...
### Saghir Bashir <br>ilustat<br>1 Dec 2018
...
Year
city  # The hash is that start of this comment #<<
```
...
### Use `ls()` to see (list) what objects you have
```{r lsObjects}
ls()
...
### We have already seen some functions (e.g, `mean()`, `seq()`)

### Function has a name and list of arguments
...
CW <- as.data.table(ChickWeight)[
    , Chick := as_factor(as.character(Chick))][
    , Diet := as.factor(Diet)][
...
```{r dropCols, echo=TRUE, eval=TRUE}
...
```{r renamevar, highlight.output=c(1)}
setnames(cw, c("weight", "LongD"), c("Weight", "Group"))
cw
```
...
### Double Click on "World-Popn.Rproj"
